...
public class TrainVoucherActivity  extends AppCompatActivity implements View.OnClickListener,  NetworkTask.Result {
   ...
   // private RecyclerView rvPaymentMethods;
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        binding = DataBindingUtil.setContentView(this, R.layout.activity_train_voucher);
        init();
        setProperties();
        setDataToViews();
        getPaymentMethods();
    ...
    private void init()
    {
        ...
        binding.titleHeader.title.setText(getString(R.string.my_trips_train_voucher));
    ...
    private void setDataToViews()
    {
        ...
        response = (TrainTicketResponse) getIntent().getSerializableExtra("view_ticket_response");


        TrainVoucher trainVoucher = new TrainVoucher(
                response.getTrip().getSelectedResult().getTrainName(),
                response.getTrip().getPnr(),
                response.getTrip().getBookingRefNo(),
                response.getTrip().getOrigin(),
                response.getTrip().getDestination(),
                response.getTrip().getTicketNumbers(),
                response.getTrip().getSelectedResult().getArrivalTime(),
                response.getTrip().getSelectedResult().getDepartureTime(),
                response.getTrip().getStatus(),
                "",
                "",
                "",
                response.getTrip().getSelectedResult().getFare().getTaxes().getTicketFare().getPrice().getAmount(),
                response.getTrip().getSelectedResult().getFare().getTaxes().getIrctcCharge().getPrice().getAmount(),
                response.getTrip().getSelectedResult().getFare().getTaxes().getAgentServiceCharge().getPrice().getAmount(),
                response.getTrip().getSelectedResult().getFare().getTaxes().getPgCharge().getPrice().getAmount(),
                response.getTrip().getSelectedResult().getFare().getTotal().getPrice().getAmount(),
                response.getTrip().getSelectedResult().getJourneyDate()
                );
        binding.setData(trainVoucher);
    ...
    public void onClick(View view) {
                ...
                public void onClick(DialogInterface dialog, int which) {
                    ...
                    switch (which) {
                        case DialogInterface.BUTTON_POSITIVE:
                            String url;
                            NetworkTask networkTask = new NetworkTask(TrainVoucherActivity.this, ID_CANCEL_TRIP);
                            networkTask.setDialogMessage(getString(R.string.please_wait));
                            networkTask.exposePostExecute(TrainVoucherActivity.this);
                            url = UIUtils.getBaseUrl(TrainVoucherActivity.this) + WebServiceConstants.urlCancelTrip;
        ...
        else if(view == binding.llPayStay)
        {
            ...
            String baseFareString="", taxString = "", totalString = "";

            try {
                baseFareString = response.getTrip().getSelectedResult().getFare().getTaxes().getTicketFare().getPrice().getAmount();
                totalString = response.getTrip().getSelectedResult().getFare().getTotal().getPrice().getAmount();

                float taxes = Float.parseFloat(response.getTrip().getSelectedResult().getFare().getTaxes().getAgentServiceCharge().getPrice().getAmount()) +
                        Float.parseFloat(response.getTrip().getSelectedResult().getFare().getTaxes().getIrctcCharge().getPrice().getAmount()) +
                        Float.parseFloat(response.getTrip().getSelectedResult().getFare().getTaxes().getPgCharge().getPrice().getAmount());
                taxString = "" + taxes;
            ...
            alertBinding.tvTaxes.setText(taxString);
